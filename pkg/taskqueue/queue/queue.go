...
func (q *TaskQueue) PopWithKey(key tqueue.TKey) tqueue.Task {
	q.mux.Lock()
	defer q.mux.Unlock()
...
func (q *TaskQueue) PopWithLimit(limit rcmgr.Limit) tqueue.Task {
	q.mux.Lock()
	defer q.mux.Unlock()
	index := -1
...
func (q *TaskQueue) Timeout(key tqueue.TKey) bool {
	q.mux.RLock()
	defer q.mux.RUnlock()
	if _, ok := q.indexer[key]; !ok {
		return false
	...
	task := q.tasks[idx]
	if time.Now().Unix()-task.GetUpdateTime() > task.GetTimeout() {
		return true
	}
	return false
}

func (q *TaskQueue) RetryFailed(key tqueue.TKey) bool {
	q.mux.RLock()
	defer q.mux.RUnlock()
	if _, ok := q.indexer[key]; !ok {
		return false
	...
	task := q.tasks[idx]
	if task.GetRetry() >= task.GetRetryLimit() {
		return true
	}
	return false
}
...
func (q *TaskQueue) RunStrategy() {
	...
	q.mux.RUnlock()
	go q.strategy.RunCollection(q, keys)
}
