...
func (p *TaskPriorityQueue) PopWithKey(key tqueue.TKey) tqueue.Task {
	p.mux.Lock()
	defer p.mux.Unlock()
	for _, queue := range p.pqueue {
		if task := queue.PopWithKey(key); task != nil {
			return task
		}
	}
	return nil
...
func (p *TaskPriorityQueue) GetPriority() []tqueue.TPriority {
	p.mux.RLock()
	defer p.mux.RUnlock()
...
func (p *TaskPriorityQueue) PopWithLimit(limit rcmgr.Limit) tqueue.Task {
	p.mux.Lock()
	defer p.mux.Unlock()
	...
	for _, queue := range p.pqueue {
		...
		task := queue.PopWithLimit(limit)
		if task == nil {
			continue
		}
		tasks = append(tasks, task)
	}
	task := p.strategy.RunPickUp(tasks)
	for _, t := range tasks {
...
func (p *TaskPriorityQueue) Timeout(key tqueue.TKey) bool {
	p.mux.RLock()
	defer p.mux.RUnlock()
	for _, queue := range p.pqueue {
...
func (q *TaskPriorityQueue) RetryFailed(key tqueue.TKey) bool {
	q.mux.RLock()
	defer q.mux.RUnlock()
	for _, queue := range q.pqueue {
		if queue.RetryFailed(key) {
			return true
		}
	}
	return false
...
func (p *TaskPriorityQueue) RunStrategy() {
	p.mux.RLock()
	defer p.mux.RUnlock()
	for _, queue := range p.pqueue {
